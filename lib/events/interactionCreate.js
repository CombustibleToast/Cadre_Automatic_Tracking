...
module.exports = {
    ...
    async execute(interaction) {
		...
		if(cooldownCheck(interaction.user.id))
            return;
        ...
        if (interaction.isChatInputCommand()) {
			...
			//Actually just do the trycatch? it will be caught there
            // if (!command) {
...
async function cooldownCheck(userId){
	...
	if(cooldowns.has(userId)){
		...
		await interaction.reply({content: `Rate limited. Wait a second and try again.`, ephemeral: true});
		return true;
	}
